...
class indicators:
    ...
    def SMA(self, windows: list[int]) -> pd.DataFrame:
        ...
        SMA = pd.concat(
            [
                self.price.rolling(window=window, min_periods=window)
                .mean()
                .rename(
                    columns={
                        ticker: f"SMA_{window}_{ticker}" for ticker in self.tickers
                    }
                )
    ...
    def EWMA(self, spans: list[int]) -> pd.DataFrame:
        ...
        EWMA = pd.concat(
            [
                self.price.ewm(span=span, min_periods=span)
                .mean()
                .rename(
                    columns={ticker: f"EWMA_{span}_{ticker}" for ticker in self.tickers}
                )
                for span in spans
            ],
            axis=1,
    ...
    def RSI(self, n: int) -> pd.DataFrame:
        ...
        loss = delta.where(delta < 0, 0).abs().rolling(window=n).mean()
        ...
        RSI = RSI.rename(
            columns={ticker: f"RSI_{n}_{ticker}" for ticker in self.tickers}
        )
    ...
    def MACD(self, short: int, long: int, signal: int) -> pd.DataFrame:
        ...
        short_ewma = self.price.ewm(span=short).mean()

        long_ewma = self.price.ewm(span=long).mean()

        MACD = short_ewma - long_ewma

        MACD = MACD.rename(
            columns={ticker: f"MACD_{ticker}" for ticker in self.tickers}
        )

        signal_line = (
            MACD.ewm(span=signal)
            .mean()
            .rename(
                columns={
                    f"MACD_{ticker}": f"signal_line_{ticker}" for ticker in self.tickers
                }
            )
        )
    ...
    def ROC(self, n: int) -> pd.DataFrame:
        ...
        ROC = (M / N * 100).rename(
            columns={f"{ticker}": f"ROC_{n}_{ticker}" for ticker in self.tickers}
        )
    ...
    def MOM(self, n: int) -> pd.DataFrame:
        ...
        MOM = self.price.diff(n).rename(
            columns={f"{ticker}": f"MOM_{n}_{ticker}" for ticker in self.tickers}
        )
    ...
    def STOK(
        ...
        STOK = (
            ...
            * 100
        ).rename(columns={f"{ticker}": f"STOK_{n}_{ticker}" for ticker in self.tickers})

        return STOK

    def STOD(self, close, low, high, n) -> pd.DataFrame:
        """
        Stochastic D
        ...
        STOK = (
            ...
            / (high.rolling(n).max() - low.rolling(n).min())
            * 100
        ).rename(columns={f"{ticker}": f"STOD_{n}_{ticker}" for ticker in self.tickers})
    ...
    def compute(self, indicator: str, **kwargs) -> None:
        ...
        if hasattr(self, indicator):
            ...
            self.df = pd.concat([self.df, TA], axis=1)
        else:
