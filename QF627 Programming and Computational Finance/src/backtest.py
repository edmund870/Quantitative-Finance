...
class backtest:
    ...
    def run(self, shares_to_buy: int = None) -> pd.DataFrame:
        """
        ...
        for i in range(1, self.trade_days):
            if self.position[i] == 1 and self.position[i - 1] == 0:
                shares[i] = (
                    (cash[i - 1] - self.commissions) / self.prices[i]
                ...
                cash[i] = cash[i - 1] + shares[i - 1] * self.prices[i]
                shares[i] = (
                    (cash[i] - self.commissions) / self.prices[i]
                    if shares_to_buy is None
                    else shares_to_buy
                )
                commissions[i] = commissions[i - 1] + self.commissions
        ...
        result["returns"] = result["total"].pct_change().fillna(0)

        result["cumulative_returns"] = (result["returns"] + 1).cumprod()
